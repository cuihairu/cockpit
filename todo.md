...
**M3 Android release 落地**（2026-09-23，单笔 feat(mobile)）：Android release 出包基础设施收口（此前 release 用 debug 签名 + 无 R8 + 无 splash 定制）。**顺带修一个真 bug**：main `AndroidManifest.xml` 缺 `INTERNET` 权限（只有 debug/profile manifest 有）——release 包联网必挂。改动面：
- **签名**（`android/app/build.gradle.kts` + `key.properties.example`）：优先读 `key.properties`（已 gitignore），四项（storeFile/storePassword/keyAlias/keyPassword）齐全才建 release 签名配置，否则回退 debug 签名——保证「暂无私钥也能 `flutter build apk --release` 出包」，上架/分发前换正式签名
- **R8 混淆 + 资源收缩**（`proguard-rules.pro`）：`isMinifyEnabled`+`isShrinkResources`，keep 规则逐项注明原因——Flutter embedding 反射入口 / dio 拦截器泛型 / flutter_secure_storage 的 Keystore·JCA 反射 / local_auth 的 FragmentActivity·BiometricPrompt / xterm 平台视图，升级依赖后复核
- **Android 12+ splash**（`values-v31`/`values-night-v31`）：`windowSplashScreenBackground`（浅 #1E88E5 / 深 #1d1d1d）+ `windowSplashScreenAnimatedIcon`，旧版本仍走 `launch_background` layer-list 回退
- **返回键/手势语义**（`home_page.dart`/`lock_gate.dart` PopScope）：非首页 tab → 回首页 tab（Material 规范），首页 tab → 退出 app；LockGate 锁定态 `canPop: false` 禁返回绕过解锁门
- **状态栏/导航栏配色**（`main.dart` SystemChrome）：浅色深图标/深色浅图标，`appBarTheme.systemOverlayStyle` 随主题切换
- **cleartext 策略**（`network_security_config.xml` ×2）：main 禁 cleartext（`usesCleartextTraffic="false"`，自签证书场景由应用内开关在 `HttpClient.badCertificateCallback` 放行——自签 ≠ 明文，两件事分开管）；debug 放行（本地 http server 调试）
- **权限最小化**：仅 `INTERNET`（文件浏览/上传走 Agent RPC + SAF 无需存储权限；生物识别由 local_auth 插件合并注入 `USE_BIOMETRIC`）
- **应用显示名**：label `cockpit_mobile` → `Cockpit`

验收边界：`flutter analyze` 0 issue、`flutter test` **112/112** 全绿（M2 时点基线 34/34，其后各批已增至 112）。真机验收项（release 包安装、splash 呈现、返回键手感、状态栏配色、cleartext 拒绝、R8 后运行、签名可升级）需真机+正式签名，挂起待验。

**M4 反代视图（只读起步）设计完成（2026-09-23）**：M2 顺延项收口（M3 专注 Android release 未及评估）。**范围**：只读三态——状态卡 + 站点列表 + 配置预览；写操作（新建/编辑/删除）留二期。**形态**（对齐 web `pages/Proxy/index.tsx` 483 行）：per-agent 端点 `/api/agents/{id}/proxy/*` 与定时任务/文件同类，**走主机能力动作单入口不进底部 tab**（底部 tab 留全局聚合视图），AppBar 标题 `反代 · {hostname}` 与 `定时任务 · {hostname}` 同款。**方案（D1-D6）**：D1 只读三端点起步——`GET proxy/status`（概览）+ `GET proxy/sites`（列表）+ `GET proxy/sites/{name}`（详情含渲染后配置全文）；PUT/DELETE 不做（写操作要 nginx -t 失败回滚语义 + 表单校验双端同规则，投入大留二期）。D2 模型对齐 agent 侧 `ProxySite` json tag（camelCase，非 Go 传统 snake）：`ProxyStatus{backend?,installed,version,confDir,siteCount,reloadMode}`、`ProxySite{name,serverNames[],upstream,scheme,tlsCert?,tlsKey?,websocket,extra}`、`ProxySiteDetail{name,site,content}`——**列表项是裁剪版**（agent `Sites()` 裁掉 tlsCert/tlsKey/extra），详情/证书路径必须走 `site.get`，不可拿列表行充当完整 ProxySite。D3 **坑位**：`status.backend` 可缺省（旧 agent 按 nginx 兜底）；`reloadMode` 三值文案映射（systemctl→"systemctl"、hot→"热加载（file provider）"、signal/其它→"nginx -s reload"）；Traefik 时 `installed` 恒 true、`version` 可能空串（容器化无二进制）、`confDir` 是动态目录、片段后缀 `.yml`（Nginx `.conf`）。D4 **Agent 挂载点三处**：`Agent.hasProxy` getter（`nginx-proxy`|`traefik-proxy` capability 任一）、`_showActions` 动作单 ListTile（icon `Icons.lan`）、trailing 高频捷径 IconButton。D5 页面抄 `cron_page.dart` 范式 A（per-agent 只读页）：`_future + setState(_reload) + RefreshIndicator + FutureBuilder` 三态，错误态包 `ListView` 才能下拉重试；状态卡用 `Card` + `Wrap` chip（mobile 无 Descriptions，按 confDir/siteCount/reloadMode 分行 chip）；配置预览用 `showModalBottomSheet` 等宽 `SelectableText`（content 含 nginx `# cockpit:meta` 头，长文可选中复制）。D6 API 扩 `ProxyApi` extension（`proxyStatus`/`proxySites`/`proxySite` 三方法，对齐 web `services/api.ts` 929-957 的 `getProxyStatus/getProxySites/getProxySite`）。**不做**：写操作表单（二期）、web 端的 `extra` 自定义片段编辑（Traefik 禁用）、差异 diff（在 Drift 页）、新建/编辑 Modal。**测试**：`proxy_page_test.dart` 三态（列表渲染含域名 Tag/协议 Tag/WebSocket 标记、空态「暂无站点」、加载失败+下拉刷新恢复）+ 详情预览 + `agents_page_test` 补 hasProxy 动作单入口断言。**验收边界**：`flutter analyze` 0 issue、`flutter test` 全绿（只增不减）、CI Mobile workflow 绿。真机验收项（真 nginx/Traefik 主机站点列表、配置预览长文渲染）挂起待验。

**M5 反代写操作（新建/编辑/删除）落地完成（2026-09-23，实现 a9c095f4）**：M4 遗留的二期项（只读三态已收口）。**范围**：站点新建/编辑表单 + 删除确认；对齐 web `pages/Proxy/index.tsx` 表单语义（读 web 实现逐字段核对，非凭记忆）。**方案（D1-D7）**：D1 **API 两方法**（`ProxyApi` 扩）：`PUT /api/agents/{id}/proxy/sites/{name}`（body 完整 ProxySite，返回 `{name,file}`）+ `DELETE /api/agents/{id}/proxy/sites/{name}`；**URL name 与 body name 必须一致**（Go 侧 `payload.Name != name` 即 400 `site name in URL and body must match`），故 apply 方法签名带显式 name 参数且与 body name 同源。D2 **表单字段严格对齐 web**（8 字段）：`name`（编辑时 disabled + tooltip「用于配置文件名 cockpit-site-<名称>.conf，创建后不可修改」；正则 `^[a-z0-9][a-z0-9_-]{0,63}$`，错文案「小写字母/数字开头，可用 - 和 _，最长 64 字符」）、`serverNames`（tags 多值，回车确认；正则 `^[A-Za-z0-9*.\-]{1,253}$`，错文案「域名只能含字母/数字/点/连字符/通配符 *」；**前端只校验非空+逐项正则，16 上限交给 Go**——web 现状如此，对齐不自创）、`upstream`（正则 `^[A-Za-z0-9.:\-]{1,253}$`，错文案「形如 127.0.0.1:3000」）、`scheme`（HTTP/HTTPS 按钮组，默认 http）、`tlsCert`/`tlsKey`（**仅 https 条件渲染**，必填文案「https 需要证书绝对路径」/「https 需要私钥绝对路径」——**前端只做 required，绝对路径格式校验留 Go**）、`websocket`（Switch，默认 false）、`extra`（4 行等宽，前端无长度限制——4096 字节由 Go 强制；**Traefik 后端禁用** + 提示「Traefik 后端不支持高级指令，留空即可」）。D3 **trim 语义对齐 web**（Go 不 trim）：name/upstream/tlsCert/tlsKey/extra 均 trim，serverNames 逐项 trim 后 filter(Boolean)，extra 空串归一为不传；**trim 后 serverNames 为空 → 全局提示「请至少填写一个域名」**（web 唯一的额外自定义校验）。D4 **错误展示两路**（对齐 web）：应用失败走**表单内 Alert**（标题「应用失败」+ 错误原文 `<pre>` 等宽——nginx -t stderr 摘要经 RPC 原样透传，Go 502 body `{"error":msg}`），**不关 Modal**；删除失败走 toast（fallback「删除失败」，响应 `error` 字段取原文）。成功文案：应用「站点 {name} 已应用」/删除「站点 {name} 已删除」。D5 **删除确认**对齐 web Popconfirm：标题「删除站点」、正文「将从 {Nginx|Traefik} 移除 {name}，确认？」、确认按钮「删除」（danger 红）、取消「取消」。D6 **权限**：`proxy:write`（PUT/DELETE，RBAC `agentSubResources["proxy/"]`）；移动端目前无 RBAC 门禁（与 web PermGuard 不同），保持一致——写按钮恒显，403 时走 D4 错误展示。D7 **UI 形态**：新建/编辑用 `showModalBottomSheet` 全屏可滚（mobile 无 Modal 620px 语义）+ `Form` + `GlobalKey<FormState>`；站点行 trailing 加编辑/删除 IconButton（既有配置预览 onTap 保留）；AppBar 右侧加「新建站点」IconButton。**不做**：Traefik extra 直通（agent 侧拒绝 `extra directives are not supported by the traefik backend`）、批量操作、配置预览改编辑（预览是渲染后产物，编辑是声明源，两回事）。**测试**：`proxy_page_test.dart` 补写操作例（新建表单校验失败文案、新建成功刷新、编辑预填+name 禁用、删除确认+成功、应用失败 Alert 展示错误原文、Traefik 时 extra 禁用）；`endpoints_test`/`models_test` 若有 API 形态断言则同步补。**验收边界**：`flutter analyze` 0 issue、`flutter test` 全绿（只增不减）、CI Mobile workflow 绿。真机验收项（真 nginx 站点下发+reload、nginx -t 失败 Alert 展示 stderr 摘要、Traefik 热加载）挂起待验。**已落地**（a9c095f4，2026-09-23）：方案全量实现，测试 117→123（新建校验失败/新建成功刷新/编辑预填+name 禁用/删除确认+成功/应用失败 Alert 错误原文/Traefik extra 禁用）。实施坑位：ListView(shrinkWrap:true) 仍懒构建致表单底部按钮 find 不到（改 SingleChildScrollView+Column）；屏幕外 FilledButton 需 ensureVisible；validator 文案需 enterText 后 pump；Icons.add 与 AppBar 新建按钮歧义（域名添加改 Icons.playlist_add）。上述真机项仍挂起待验。

**M6 mobile 测试覆盖率收口（2026-09-24，单笔 test(mobile) 75113d38）**：对齐 Go 侧覆盖率收口（§覆盖率收口 2026-09-23）的 mobile 版。**状态**：97.23%（缺 49 行）→ **99.83%**（1764/1767，142 测试全绿，`flutter analyze` 0 issue）。逐行补测：lock_gate 五分支（通过/未通过/异常/挂起期间卸载/强制 pop didPop）、terminal 真 WS 全链路（连通/下行/键盘 onOutput 转发/服务端 close 帧 + wss 自签分支用 test/fixtures 自签证书 bindSecure + ticket 返回前退出页面的 !mounted 用例）、settings_state token 刷新成功重放链路（401→refresh→重试落盘）、dashboard apiProvider loading（**Completer 永挂 override**——async 闭包微任务内完成捕捉不到 loading 帧）、audit 下拉刷新分页追加语义（hasMore 恰好取满终止预取循环）、proxy 写操作 9 分支（错误 SnackBar/删除失败/HTTPS 证书校验/域名 chip 编辑）、files/backups 大容量单位格式化、agents 代理入口、home PopScope 返回。**剩余 3 行登记 `tool/known_uncoverable.txt`（附理由）**：`dashboard_page.dart:19`（const 构造行——loading 分支运行时已由永挂用例验证，lcov 对 const lambda 漏计）、`terminal_page.dart:56`（await 挂起行——若 !mounted 分支未走，后续 defunct setState 会使测试失败，用例通过即证明命中）、`terminal_page.dart:73`（WS stream onError——IOWebSocketChannel.connect 握手失败走 ready future 不进 stream，真实 TCP 异常 fake-async 环境不可注入，已实验证实）。**门禁**：`tool/coverage_check.sh` 解析 lcov 未覆盖行与登记表对比（comm -23 未登记即失败 / -13 过期提醒），跑通输出「有效覆盖 100%」；顺带修 LF/LH 汇总解析 bug（其值无空白分隔须 `-F:`，原 awk 默认分隔符下恒为 0 除零）。**前置真 bug**：client.dart 401 刷新递归（refresh 自身 401 未检查 skipRefresh extra，无限递归）已单独 fix 提交（d50d3c22）。**坑集沉淀**：testWidgets 真网络四要素（bind/listen 进 runAsync、HttpOverrides 保存-置 null-还原、tap 不进 runAsync、wss 真 TLS）、main() 冒烟测试独占文件（跨测试 Ticker 污染，单跑绿合跑挂）、dio 测试 server 必须设 JSON Content-Type（否则 data 解析成 String 假阴性）、fake zone WS 握手推迟致 accept 顺序颠倒（close 帧发 sessions.first）。CI 口径说明：mobile.yml 已挂 Coverage gate 步骤（`flutter test --coverage` + `coverage_check.sh --skip-test`，6156dd77，CI 首跑通过与本地完全对齐），未覆盖行未登记即 CI 红；99.9% 阈值是 Go 侧 agent-test.yml 的口径。

**M7 web 测试覆盖率收口（2026-09-25，三笔 e56db9f3 / 459f2c7f / e9a3ea35）**：对齐 Go（阈值 100%）与 mobile（M6 台账）的第三端收口。**前置状态**：主机崩溃重启后由 0c8a48bc 快照恢复，本会话在其上完成剩余缺口逐项判定与收口。**实测数字**：全量 79 文件 897 用例全绿；行覆盖 99.49%（4924/4949）、分支覆盖 98.02%（3754/3830），88 个未覆盖点全部登记 `web/tool/known_uncoverable.txt`（四类附理由：防御性早退 selectedAgent/ref/空参、jsdom 不可注入时序——15s 连接超时回调/CONNECTING 态清理/rAF、无 UI 入口或数据演进防御——表单前置拦截/issuer 下载无入口、v8 伪缺口——行为双向实证但 JSX 内联箭头/promise 回调链/Table columns 内联 render 三元计数为 0），门禁输出「有效覆盖 100%」。**补测尾巴**（判定后仅 2 处值得补）：Docker 危险操作确认用例补 500ms 防抖 refetch 断言（onSuccess setTimeout 延迟刷新，此前未等到位）；Drift mismatch 展开表补 hostname 明细断言「主机名」映射。**判定中推翻的两个「可补」**：Proxy 147 空域名运行时校验——表单 required + validator 前置拦截，onFinish 触发时必非空，防御死代码；Acme 186 下载文件名 issuer 分支——download() 仅被 cert/key 两按钮调用，issuer 无 UI 入口。**门禁三坑**：① vitest CLI `--coverage.include='src/**'` 覆盖 config 的 ts/tsx 白名单，把 src/assets/.gitkeep 当源码解析，remapCoverage 阶段 PARSE_ERROR 且 lcov 静默不落盘（EXIT=1 假象是测试失败、真因在 include）；② coverage 插桩 + 8 worker 下 Backups 重表单用例实测 63.7s > 60s testTimeout，整跑 EXIT=1 时 lcov 不落盘——放宽 120s 根治（vitest 失败时不写 coverage 产物，看到 lcov 缺失先查测试失败再查插桩）；③ coverage_check.sh 的 `sort -u` 默认 locale 排序与 comm 字节序期望不一致报 not in sorted order——`export LC_ALL=C` 根治（Go/mobile 脚本未触发是因 lcov 短、边界行未踩中）。**CI 接入**：test.yml web job 换 `bash tool/coverage_check.sh`（跑 coverage + 台账校验一体），未登记缺口即红；本地 `--skip-test` 口径与 CI 完全一致。**三端终态**：Go 语句 100%（agent-test.yml 阈值 100%）、mobile 行 99.83%（3 行登记）、web 行 99.49% + 分支 98.02%（88 条登记，四类理由）——「有效覆盖 100%」口径在三端一致。
...
- **远控会话录制**：终端输出录制，补强远控审计（当前审计只有会话开始/结束）。→ 方案参考：Guacamole 会话录制、Teleport 会话粒度审计。✅ **M3 桌面 .guac 录制收集与 Web 回放设计完成（2026-09-23）**：Guacamole 阶段 2 首项（音频通道/文件传输按投入产出比后置）。**为什么先做回放**：guacd 已自动录制 .guac（阶段1「白捡」红利已实现，api_guacamole.go 的 connect 指令带 recording-path/recording-name），只差「收集→元数据→REST→回放」四环即可补上 recording-design.md 的「不做」缺口（VNC/RDP 桌面流录制），补强审计闭环；音频在办公场景价值低（设计风险章节自述）、文件传输与 file-manager-design.md 路线重叠。**方案（D1-D6）**：D1 复用 TerminalRecording 表 + recordingsDir 不建新表——.guac 与 .cast 同目录（`<db>/recordings/<sid>.guac|.cast`），表加 `Format` 字段（`cast`|`guac`，GORM AutoMigrate 零迁移，空值视作 cast 兼容旧数据）；录制索引语义完全一致（谁/何时/连哪台/多久/多大），格式只是内容形态，两套表是重复真相源。D2 `.guac` 收集时机 = 会话结束（closeGuacamoleSession）：guacd 卷 `<sid>.guac` → recordingsDir（跨分区用 copy+remove 非 rename），再 FinishTerminalRecording 回填 duration/bytes；会话开始仍立即登记（Format=guac）保持与 .cast「进行中也在列」语义一致，异常终止的孤儿 .guac 不进列表（可接受的罕见场景）。D3 **guacd 录制路径可配且两侧同路径**：`GUACD_RECORDING_PATH`（默认 `/var/lib/guacamole`）同时作 guacd 的 recording-path 参数与 server 的收集源——同机部署同路径即同目录，Docker 部署把同一卷挂到两侧同路径（deployments/guacd 的 guacd-recordings 卷本就挂该点，server 容器同挂即可）；server 与 guacd 视角合一避免双配置漂移。D4 REST 按 Format 分流（URL `/{sid}/cast` 不变，语义本就是「取录制内容」）：handleRecordingCast 按 rec.Format 定后缀，前端零改动；删除/清理/异地归档同样按 Format 定后缀。D5 前端回放用 `Guacamole.SessionRecording`（common-js 自带，照抄官方不自由发挥——内部建 PlaybackTunnel + Client、自动 keyframe 帧索引、play/pause/seek/getDisplay 全备）：Recordings 页按 format 分流——cast 仍用既有 xterm 回放器（开放格式，脱离 Guacamole 可回放），guac 用 SessionRecording；两者互补（终端开放格式、桌面 Guacamole 格式，与 D5「chunk/base64 照抄官方」同纪律）。D6 retention/清理/异地归档复用现有 Setting（recording.enabled/retention_days/remote_dest 三键语义不变，桌面录制同样受控）。**不做**：音频（阶段 2 次项）、文件传输（与 file-manager 路线重叠）、.guac 转开放格式（官方 guacenc 可离线转，Web 侧不重复造）、实时旁观。**真机验收项与 grdp 路线去留仍挂起等用户拍板，不自行决定**。✅ **M4 桌面音频通道（静音控制）设计完成（2026-09-23）**：Guacamole 阶段 2 次项（回放器已收口）。**关键事实（读 common-js 1.5.0 源码核实）**：音频流走 Guacamole 协议的 `audio,<streamIndex>,<mimetype>;` 流创建指令 + `blob` 数据流，**自动经 Go 网关的字节管道透传，网关零改动**；客户端用 `onaudio` 回调或 `Guacamole.AudioPlayer.getInstance(stream, mimetype)` 播放，内置 `RawAudioPlayer` 支持 `audio/L8`/`audio/L16`（raw PCM，需 rate/channels 参数）。**方案（D1-D4）**：D1 Go 网关零改动——音频流本就在字节管道里；`guacConnectArgs` 不传 `disable-audio`（guacd 默认启用音频），设计骨架里「connect 指令的 audio 参数」表述实为服务端主动推 `audio` 流（已按源码核实修正），架构上零死角成立。D2 **静音控制**（本项唯一改动面）：`Guacamole.AudioContextFactory.getAudioContext()` 是单例，`AudioContext.suspend()/resume()` 全局静音所有音频流——零自定义播放器（RawAudioPlayer 内部直连 `context.destination`，从外部插 GainNode 必须照抄其播放逻辑，属「自由发挥」禁区）；GuacamoleModal 的 RemoteToolbar `extraActions` 加静音按钮（与 VNCModal 的 CAD 按钮同位）。D3 音量滑杆留后续：插 GainNode 需自定义 AudioPlayer（照抄 RawAudioPlayer 的 ArrayBufferReader→AudioBuffer→BufferSource→destination 播放链路再插 GainNode），投入中产出低（办公场景音量调节价值有限），不值。D4 编解码对齐留真机验收：`RawAudioPlayer` 只认 L8/L16；guacd 的 RDP 音频（FreeRDP rdpsnd）实际发什么 mimetype 需真机验证（不依赖真机的部分无法确定 guacd 是否转码到 L16）。**不做**：音量滑杆（D3）、iOS 手势解锁引导（需真机验证 Safari 行为）、音频录制（无审计价值）。✅ M1 完成（2026-09-15）：`docs/guide/recording-design.md`；server 侧挂在 agent→浏览器转发管道（HandleTerminalData）输出流录制，agent 零变更；asciinema v2 `.cast` 格式落盘（`data/recordings/<sessionID>.cast` 0600）+ `TerminalRecording` 元数据表（结束时回填时长/字节，Close 幂等覆盖双结束出口）；只录输出不录输入（输入含密码）；Setting `recording.enabled`（默认开）+ `recording.retention_days`（默认 7，0=永久，挂 cleanupLoop 小时节流清理）；REST `/api/recordings` 列表/取内容/删除（取内容与删除记审计，列表不记）；Web `/recordings` 页面（列表含进行中标记 + 自研 xterm 回放器 1/2/4/8x 倍速 + 下载/删除）。剩余（后续版本）：VNC/RDP 桌面流录制、实时旁观。✅ M2 完成（2026-09-19）：录制归档异地 + 配置入口（`recording-design.md` D14-D19）——Setting `recording.remote_dest`（写时校验同 server 备份 M2 D13，读时脏值视为未配置）；复用抽出的 `rcloneCopyLocalFile` 共用执行器（server 备份与录制同源，含 WaitDelay 强断管道）；录制 Close 回填元数据后异步推送（不拖会话出口路径），失败发 `recording.remote-failed` 通知（本地档在，retention 窗口内可补推）；`POST /api/recordings/{sid}/sync-remote` 手动补推（sid UUID 严格校验 + 审计）；`GET/PUT /api/recordings/config` 补 M1 缺口（enabled/retention_days 此前无写入口）；web /recordings 页配置行 + 列表补推按钮。剩余：rclone 真远端推送真机验证。
